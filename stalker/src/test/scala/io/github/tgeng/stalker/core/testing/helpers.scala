...
trait Helpers {
  def (ft: FTerm) tt (using LocalFtCtx, Namespace): Term = ft.toTt match {
    case Right(t) => t
    case Left(e) => throw Exception(e.toString)
  }

  def (t: Term) fe (using LocalTfCtx, Namespace): FTerm = t.toFe

  def (t: Term) whnf (using LocalTfCtx, Context, Namespace, Signature) = t.toWhnf match {
    case Right(w) => w
