  ...
  override def load(key: K, fingerprint: F) = {
    cache.load(key) match {
      ...
      case _ => {
        ...
        cache.load(key) match {
          case Some(v, f) if (f == fingerprint) => v
          case _ => throw IllegalStateException(s"Fail to load value for key $key and fingerprint $fingerprint.")
        }
      }
    }
  }
...
case class FingerPrint private(private val bytes: Array[Byte]) {
  ...
  override def equals(that: Any) = that match {
    case that: FingerPrint => that.hashCode == hashCode && Arrays.equals(that.bytes, bytes)
    case _ => false
  }
